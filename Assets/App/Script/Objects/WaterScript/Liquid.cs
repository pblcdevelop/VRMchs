...
public class Liquid : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (deltaTime != 0)
        {
            ...
            wobbleAmountToAddX = Mathf.Lerp(wobbleAmountToAddX, 0, (deltaTime * _recovery));
            wobbleAmountToAddZ = Mathf.Lerp(wobbleAmountToAddZ, 0, (deltaTime * _recovery));



            // make a sine wave of the decreasing wobble
            pulse = 2 * Mathf.PI * _wobbleSpeedMove;
            sinewave = Mathf.Lerp(sinewave, Mathf.Sin(pulse * time), deltaTime * Mathf.Clamp(velocity.magnitude + angularVelocity.magnitude, _thickness, 10));
            ...
            wobbleAmountToAddX += Mathf.Clamp((velocity.x + (velocity.y * 0.2f) + angularVelocity.z + angularVelocity.y) * _maxWobble, -_maxWobble, _maxWobble);
            wobbleAmountToAddZ += Mathf.Clamp((velocity.z + (velocity.y * 0.2f) + angularVelocity.x + angularVelocity.y) * _maxWobble, -_maxWobble, _maxWobble);
        }
    ...
    void UpdatePos(float deltaTime)
    {
        ...
        if (CompensateShapeAmount > 0)
        {
            ...
            pos = worldPos - transform.position - new Vector3(0, _fillAmount - (comp.y * CompensateShapeAmount), 0);
        }
        else
        {
            pos = worldPos - transform.position - new Vector3(0, _fillAmount, 0);
        }
